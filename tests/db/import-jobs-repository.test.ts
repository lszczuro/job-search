...
describe("createImportJobsRepository", () => {
  ...
  it("reuses an active refresh job before creating a duplicate", async () => {
    ...
    sqlite
      .prepare(
        `
          INSERT INTO import_jobs (kind, status, requested_by, payload, created_at, started_at)
          VALUES (?, ?, ?, ?, ?, ?)
        `
      )
      .run(
  ...
  it("fails an orphaned running refresh job and creates a new pending one", async () => {
    sqlite = new Database(":memory:");
    sqlite.exec(`
      CREATE TABLE import_jobs (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    const repository = createImportJobsRepository(sqlite);
    const job = await repository.createOrReuseRefreshJob("scheduled_refresh", "system", {
      now: "2026-04-09T10:00:00.000Z"
    });
    const previousJob = sqlite
      .prepare(`SELECT status, error_message, finished_at FROM import_jobs WHERE id = 1`)
      .get() as { status: string; error_message: string | null; finished_at: string | null };

    expect(previousJob.status).toBe("failed");
    expect(previousJob.error_message).toBe("Worker interrupted before refresh job completed");
    expect(previousJob.finished_at).toBe("2026-04-09T10:00:00.000Z");
